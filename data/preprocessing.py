...
for subdir in tqdm(os.listdir(parent_input_folder)):
    ...
    for filename in tqdm(os.listdir(input_folder)):
        if filename.endswith(".tif"):
            ...
            output_filepath = os.path.join(
                output_folder, filename.replace(".tif", ".npy")
            )
...
# +
parent_input_folder = os.path.join(basedir, "EuroSAT_MS_NPY")
parent_output_folder = os.path.join(basedir, "EuroSAT_MS_NPY_wo_B10")
...
for subdir in tqdm(os.listdir(parent_input_folder)):
    ...
    for filename in tqdm(os.listdir(input_folder)):
        if filename.endswith(".npy"):
            ...
            image_data_no_b10 = np.delete(
                image_data, 9, axis=0
            )  # Remove B10 band (assuming it's the 10th band)
...
# Load the .npy file
image_data = np.load(
    os.path.join(parent_output_folder, "AnnualCrop/AnnualCrop_1.npy")
)
...
def process_images(source_folder, dest_folder):
    for root, _, files in tqdm(list(os.walk(source_folder)), desc='Processing directories'):
...
source_folder = (
    os.path.join(basedir, "EuroSAT_MS_NPY_wo_B10")
)
ordered_folder = (
    os.path.join(basedir, "EuroSAT_MS_NPY_wo_B10_ordered")
)
