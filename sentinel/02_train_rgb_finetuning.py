...
sub_dirs = [
    sub_dir
    for sub_dir in os.listdir(path_to_train)
...
# parameters for CNN
if use_vgg:
    base_model = VGG(include_top=False, weights="imagenet", input_shape=(64, 64, 3))
else:
    base_model = DenseNet(
...
train_generator = train_datagen.flow_from_directory(
    path_to_train, target_size=(64, 64), batch_size=batch_size, class_mode="categorical"
)
...
tensorboard = TensorBoard(
    log_dir="./logs", write_graph=True, write_images=True, update_freq="epoch"
)
...
earlystopper = EarlyStopping(
    monitor="val_categorical_accuracy", patience=50, mode="max"
)
model.fit(
    train_generator,
    steps_per_epoch=50,
